...
usage() {
  cat <<'EOF'
Usage: create-web-project.sh [PROJECT_NAME] [GROUP_ID] [ARTIFACT_ID] [PACKAGE_NAME] [JAVA_VERSION]
Options:
...
>&2 echo "Creating Spring Boot web application with Boot=$BOOT_VERSION, Java=$JAVA_VERSION"
